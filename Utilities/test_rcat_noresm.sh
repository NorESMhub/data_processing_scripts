...
tstring="1:200005xx"
USEICEFILENAMES="yes"; dates="$(get_ice_hist_file_info ${ice_file})"
check_test "CICE dates test filename" "${dates}" "${tstring}"
tstring="1:20000531"
USEICEFILENAMES="no"; dates="$(get_ice_hist_file_info ${ice_file})"
check_test "CICE dates test file" "${dates}" "${tstring}"
...
years="$(get_range_year ${dates} ${lnd_file} | tr '\n' ':')"
check_test "CTSM year set test" "${years}" "ERROR 21: get_range_year: Multiple years found in ${lnd_file}:ERROR:"
tstring="1:185904xx"
...
tfile="foo.nc"
year="$(get_range_year ${frames} ${tfile} | tr '\n' ':')"
check_test "multi-year range year test" "${year}" "ERROR 21: get_range_year: Multiple years found in ${tfile}:ERROR:"
...
month="$(get_range_month ${frames})"
check_test "multi-month range month test" "${month}" "INTERNAL ERROR 17: get_range_month: Multiple months found in file"
frames="2:${tyear}${tmonth}28:$((tyear + 1))${tmonth}28"
month="$(get_range_month ${frames} ${tfile})"
check_test "multi-year range month test" "${month}" "INTERNAL ERROR 17: get_range_month: Multiple years found in ${tfile}"
...
lnd_file="${testdir}/lnd_test_file.clm2_0001.h1.2000-12-27-00000.nc"
tdate="$(get_file_date ${lnd_file} lnd yearly | tr '\n' ':')"
check_test "lnd yearly get_file_date test" "${tdate}" "ERROR 21: get_range_year: Multiple years found in ${lnd_file}:ERROR:"
tdate="$(get_file_date ${ocn_file} ocn yearly)"
check_test "ocn yearly get_file_date test" "${tdate}" "1850"
...
check_test "lnd monthly get_file_date test" "${tdate}" "INTERNAL ERROR 17: get_range_month: Multiple years found in ${lnd_file}"
tdate="$(get_file_date ${ocn_file} ocn monthly)"
check_test "ocn monthly get_file_date test" "${tdate}" "1850:01"
...
check_test "bad component yearly get_file_date test" "${tdate}" "ERROR 07: get_file_date: Unrecognized component type, 'foo'"
tdate="$(get_file_date ${atm_file} "atm" "squirrelly")"
check_test "invalid merge type get_file_date test" "${tdate}" "ERROR 09: get_file_date: Unrecognized merge type, 'squirrelly'"
tdate="$(get_file_date "frankly_i_donot_exist.nc" "atm" "monthly")"
check_test "no file yearly get_file_date test" "${tdate}" "ERROR 19: get_file_date: File does not exist, 'frankly_i_donot_exist.nc'"
